...
class DfMagics(Magics):
    ...
    @magic_arguments.argument(
        "--on",
        action="store_true",
        help="Turn on persistent df. All the following cells will show the space usage for the partition until turned off.",
    )
    @magic_arguments.argument(
        "--off", action="store_true", help="Turn off persistent df"
    )
    @magic_arguments.argument(
        "-p",
        "--path",
        default="/home/jovyan",
        help="Sets the path to the partition to check",
    )
    @magic_arguments.argument(
        "--raw", action="store_true", help="Returns the raw data object."
    )
    @magic_arguments.argument(
        "-v",
        "--verbose",
        action="store_true",
        help="Prints off additional text for debugging.",
    )

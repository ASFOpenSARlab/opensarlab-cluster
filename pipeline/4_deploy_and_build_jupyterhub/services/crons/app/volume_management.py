...
def delete_volumes(
    ...
    try:
        ...
        for vol in vols:
            ...
            if not pvc_name:
                log.warning(f"Volume '{vol_id}' not tagged 'kubernetes.io/created-for/pvc/name'. Skipping....")
                continue

            # Do not delete the Hub DB!!
            if pvc_name == 'hub-db-dir':
                log.warning(f"Volume '{vol_id}' is tagged 'hub-db-dir' found. Skipping....")
                continue
            ...
            if _get_tags(vol, 'jupyter-volume-stopping-time'):
                log.warning(f"Volume '{vol_id}' is tagged with 'jupyter-volume-stopping-time' which is an old schema and is not useable. Skipping...")
                continue

            # Get last stopped tags
            server_stop_time = _get_tags(vol, 'server-stop-time')
            if not server_stop_time:
                log.warning(f"Volume '{vol_id}' is not tagged with server_stop_time and is not useable. Skipping...")
                continue
            try:
                server_stop_time = datetime.datetime.strptime(server_stop_time, '%Y-%m-%d %H:%M:%S+00:00')
            ...
            if not volume_delete_time:
                log.warning(f"Volume '{vol_id}' is not tagged with volume_delete_time and is not useable. Skipping...")
                continue
            try:
                volume_delete_time = datetime.datetime.strptime(volume_delete_time, '%Y-%m-%d %H:%M:%S+00:00')
            except ValueError as e:
                log.error(e)
                errors_found.append({"volume_id": str(vol_id), "error_msg": str(e)})
                continue
            ...
            if server_stop_time > volume_delete_time:
                log.warning(f"Volume '{vol_id}' has a volume_delete_time value younger than server stopping time. Skipping...")
                continue
            ...
            if ignore_snapshot_requirement:
                ...
                log.info("Ignoring snapshots. Volume will be possibly deleted even if snapshot is not present.")
            else:
                # Get snapshot
                ...
                if len(snap) == 0:
                    ...
                    if len(snapshots_too_old) == len(snap):
                        log.info(f"No snapshots found newer than {days_till_too_old} days old. Will not delete volume '{vol_id}'.")
                    else:
            ...
            log.info(f"do_deactivate: {do_deactivate}, has_valid_snapshots: {has_valid_snapshot}, is_available: {is_available}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Check usage status of volumes in lab deployment and delete as needed.')
    ...
    parser.add_argument('--portal-domain', help='Domain of Portal (including https://)', dest='portal_domain', required=True)
    ...
    parser.add_argument('--ignore-snapshot-requirement', help='Ignore if backup snapshot exists and possibly delete volume anyway.', dest='ignore_snapshot_requirement', action='store_true', required=False)

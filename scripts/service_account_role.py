...
The OIDC provider should have already been created before this script was run:
...
class ServiceAccountRole(object):
    ...
    def create_service_account(self):
        ...
        try:
            ...
            self.k8s.create_namespaced_service_account(namespace=self.sa_namespace, body=body)
    ...
    def create_iam_role(self):
        ...
        """.strip()
        try:
            self.iam.create_role(RoleName=self.iam_role_name, AssumeRolePolicyDocument=trust_json)
    ...
    def add_inline_policy_to_iam_role(self):
        """
        aws --profile ${PROFILE} iam put-role-policy --role-name ${IAM_ROLE_NAME} --policy-name ${IAM_POLICY_NAME} --policy-document file://service_account_policy.json
        rm policy.json
        """
        print("Add inline policy to role...")

        self.iam.put_role_policy(RoleName=self.iam_role_name, PolicyName=self.iam_policy_name, PolicyDocument=self.inline_policy)
    ...
    def associate_iam_role_with_service_account(self):
        ...
        self.k8s.patch_namespaced_service_account(name=self.sa_name, namespace=self.sa_namespace, body=body)

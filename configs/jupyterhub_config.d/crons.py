...
try:
    ...
    with open("/usr/local/etc/jupyterhub/custom/meta.yaml", mode='w') as f:
        f.write(meta)

    # Make file executable
    os.chmod('/usr/local/etc/jupyterhub/custom/meta.yaml', 0o755)
    ...
    # Setup crontab for volume killer
    job2 = cron.new(command='python3 /usr/local/etc/jupyterhub/custom/delete_volumes.py > /proc/1/fd/1 2>&1')
    job2.hour.on(12)  # 12 UTC, 3am AKST
    job2.minute.on(0)
    job2.enable()

    # Setup crontab for snapshot killer
    job3 = cron.new(command='python3 /usr/local/etc/jupyterhub/custom/delete_snapshot.py > /proc/1/fd/1 2>&1')

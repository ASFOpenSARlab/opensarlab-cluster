...
def parse():
    ...
    parser.add_argument('-i', action='store', default='WAVELET-INV-xval.h5',
            dest='iname', help='Inverted wavelet coefficients. \
                    Default: WAVELET-INV.h5')
    parser.add_argument('-o', action='store', default='WS-xval.h5',
            dest='oname', help='Final time-series HDF5 file.  \
                    Default: WS-xval.h5') 
    parser.add_argument('-nchunk', action='store', default=40, dest='nchunk', help='Chunk of slices that will be loaded into memory at any given time. Default: 40')
    parser.add_argument('-nproc', action='store', default=1, dest ='nproc', help='Number of threads. Default: 1')
...
if __name__ == '__main__':
    ...
    imodel = odat.create_dataset('model',(Nsar,nm,crop[2],crop[3]),'f')
    ...
    ierr   = odat.create_dataset('error',(H.shape[0],crop[2],crop[3]),'f')
    ierr.attrs['help'] = 'Uncertainty in reconstructed time-series.'
    ...
    nproc = inps.nproc
    progb = ts.ProgressBar(maxValue=Nsar)

    for kkk in xrange(Nsar):
        for grpnum in xrange(len(ngrps)-1):
            pars.start = ngrps[grpnum]
            ninds = ngrps[grpnum+1] - ngrps[grpnum]
            pars.igram[0:ninds,:,:] = wvlt[kkk,ngrps[grpnum]:ngrps[grpnum+1],:,:]
            ...
            for k in xrange(ninds):
                imodel[kkk,ngrps[grpnum]+k,:,:] = commonarr[k,crop[0]:crop[0]+crop[2],crop[1]:crop[1]+crop[3]]
    ...
    params = np.zeros((Nsar,H.shape[0],crop[3]))
    for k in xrange(crop[2]):
        for mm in xrange(Nsar):
            coeff = imodel[mm,:,k,:]
            iwt = np.dot(H,coeff)
            params[mm,:,:] = iwt - iwt[masterind,:]
        
        its[:,k,:] = np.median(params, axis=0)
        ierr[:,k,:] = np.std(params, axis=0, ddof=1)

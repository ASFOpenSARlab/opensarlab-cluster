...
from .handlers import (AuthorizationHandler, ChangeAuthorizationHandler,
                       ChangePasswordHandler, LoginHandler, SignUpHandler, GroupHandler)
from .orm import UserInfo, GroupInfo, UserGroupMapInfo
...
class NativeAuthenticator(Authenticator):
    ...
    def get_groups_by_user_id(self, user_id):
        group_ids = UserGroupMapInfo.find_all_group_ids_by_user_id(self.db, user_id)
        group_names = [GroupInfo.find_group_name_by_group_id(self.db, gid) for gid in group_ids]
        return group_names

    def get_groups_by_username(self, username):
        """Get list of groups by username"""
        user = self.get_user(self.normalize_username(username))
        groups = self.get_groups_by_user_id(user.id)
        return groups
